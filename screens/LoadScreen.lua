local LoadScreen = BaseScreen:new({
    ...
    update = function(self, dt)
        ...
        if self.loaded > 0 and self.loaded % 5 == 0 then
            print(string.format("LoadScreen: %d/%d (%.1f%%) - %s",
                self.loaded, self.total, self.progress * 100, self.currentName))
        end
    ...
    render = function(self)
        ...
        -- DrawManager:fillText(loaded .. "/" .. total .. " loaded",
        ...
        -- Barra de progreso
        DrawManager:fillRect(barX, barY, barWidth * progress, barHeight, { color = "#005C05" })
        -- Borde de la barra
        DrawManager:strokeRect(barX, barY, barWidth, barHeight, { color = "white" })

        -- Mostrar qué se está cargando actualmente
        -- if self.currentName and self.currentName ~= "" then
        --     DrawManager:fillText("Loading: " .. self.currentName,
